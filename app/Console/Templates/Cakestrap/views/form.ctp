	...
	<div id="page-content" class="col-sm-12">
		...
		<div class="<?php echo $pluralVar; ?> form p_form">
			...
			<?php echo "<?php echo \$this->Form->create('{$modelClass}', array('inputDefaults' => array('label' => false), 'role' => 'form')); ?>\n"; ?>
				<fieldset>
					<h2><?php printf("<?php echo __('%s %s'); ?>", traductor($action), $singularHumanName); ?>
...
<?php
			...
			$divs = 2;
			 $count = (int)count($fields)/$divs;
			for ($i = 0; $i <= $count; $i++):
			for ($j = ($i*$divs)+1; $j <= $divs+($i*$divs); $j++):

			if ($j >= count($fields)): break;endif;
			$field = $fields[$j];
				if (in_array($field, ['usuario_id','fecha_carga', 'tabla', 'tabla_id', 'persona_id'])) continue;
				...
				if (strpos($action, 'add') !== false && $field == $primaryKey) {
					...
					echo ($schema[$field]['type'] == 'datetime' || $schema[$field]['type'] == 'date')? '\'type\'=>\'text\',' : ''; 
		...
		foreach ($relations as $alias => $details):
			...
			<div class="related">
				...
				<div class="actions">
					...
					"array('controller' => '{$details['controller']}', 'action' => 'add',h(\$this->data['{$modelClass}']['id'])),".
